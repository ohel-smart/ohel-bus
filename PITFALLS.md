...
The bot also actively calls **into** this site: right after sending its WhatsApp summary, it HTTP-triggers `api/daily-email.js` (see that section above) so the manager email follows the same trigger timing. That endpoint is deduplicated by a Firestore state doc, not by the two services coordinating directly — if you ever add a third trigger path for it, make sure it goes through the same dedupe doc.
