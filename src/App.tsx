...
export default function App() {
    ...
    const html = `<!DOCTYPE html><html lang="he"><head><meta charset="utf-8">
      ...
      <style>
        ...
        * { box-sizing: border-box; }
        ...
        .report-header { display: flex; align-items: center; gap: 16px; border-bottom: 3px solid var(--gold); padding-bottom: 16px; margin-bottom: 20px; }
        .logo { height: 44px; }
        ...
        @media print { .lang-toggle { display: none; } body { padding: 0; } }
      </style></head>
      <body>
  ...
  const handleExportDispatcherPdf = () => {
    ...
    const dispatcherScans = scans
      .filter(s => (s.dispatcherName || '').replace(' (סדרן)', '') === selectedDispatcherForPdf)
      .slice()
      .sort((a, b) => new Date(b.scannedAt).getTime() - new Date(a.scannedAt).getTime());
