...
### `api/daily-email.js` — retired, kept only as a manual fallback

**Retired 2026-09-20.** This Vercel Function used to email the manager a daily rides summary via Resend, self-triggered by a GitHub Actions workflow pinging it every 10 minutes. Both the automatic Vercel Cron path and coupling the trigger to the WhatsApp bot were considered and rejected early on (Vercel's Hobby plan cron is once/day with up to ±59min of imprecision; the bot's uptime shouldn't gate the email) — but the GitHub Actions pinger approach itself was superseded by a self-contained Google Apps Script project ("Ohel Bus - Daily Email Trigger") that reads the same Firestore data directly and sends via `GmailApp` instead of Resend. The `.github/workflows/daily-email-trigger.yml` pinger has been deleted, so this endpoint is no longer called automatically — it's left in the repo only as an inert reference/manual-emergency-fallback, not as part of normal operation.

Do not re-add automatic pinging of this endpoint without also addressing the reason it was retired alongside the pinger removal: this file and the Apps Script both read-then-write the same Firestore doc (`bot_state/daily_email`, field `lastEmailDate`) with no transaction, so running both on a schedule at once is a duplicate-send race, not just redundant. That race is separate from (and was found alongside) a since-fixed comparison-operator bug in this same self-heal logic that caused real duplicate-message spam on Erev Shabbat before being fixed identically here, in the WhatsApp bot, and in the Apps Script — see git history around 2026-09-18/20 for the incident.

Still uses `firebase-admin` (not the client SDK) with a service-account key from `process.env.FIREBASE_KEY`, and reimplements a few helpers (Hebrew date, NY date, HTML-escaping, the Erev Shabbat/Erev Yom Tov trigger-moment check) locally rather than importing from `src/` — if this is ever revived, re-sync those against `src/` first, since they've had no reason to stay in sync since retirement.

The endpoint isn't behind a real secret — it accepts any caller whose `user-agent` includes `vercel-cron` (no `CRON_SECRET` env var is configured). This is publicly visible in this repo's source, so treat it as a soft gate, not real authentication, if it's ever called again.
